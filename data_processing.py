...
players_df.rename(columns={"c": "team", "ac": "playerid",
                  "na": "name", "SEASON": "season"}, inplace=True)
players_df['playerid'] = players_df['playerid'].apply(lambda x: x.strip())

# Instantiate all players
players = [Player(player[0], player[1]) for player in players_df.groupby(
    ["playerid", "name"])["playerid", "name"].apply(list).to_dict()]
...
shots = [Shot(shot['ID_PLAYER']+str(shot['UTC']), shot['ID_PLAYER'], shot['COORD_X'],
              shot['COORD_Y'], shot['ID_ACTION'], shot['SEASON']) for shot in points_df.to_dict('records')]
...
for i in np.arange(court_bounds['x_low'], court_bounds['x_up'], xstep):
    for j in np.arange(court_bounds['y_low'], court_bounds['y_up'], ystep):
        bin_def = {"x_bound_up": i+xstep, "y_bound_up": j+ystep, "x_bound_down": i, "y_bound_down": j
                   }
...
totals_cols = ['2p', '3p', 'ft', '3pct', '2pct', 'ftpct', '2px', '2py', '3px', '3py',
               '2pTot', '3pTot', 'ftTot', '3pctTot', '2pctTot', 'ftpctTot', 'player', 'season']
...
df_2p = totals.loc[totals['2px'].notnull(
), ['2p', '2pct', '2px', '2py', '2pctTot', 'player', 'season']]
# create df specific for 3 point shots
df_3p = totals.loc[totals['3px'].notnull(
), ['3p', '3pct', '3px', '3py', '3pctTot', 'player', 'season']]

# change column names to generic
df_2p.columns = ['nr_player_shots', 'pct_player',
                 'x_agg', 'y_agg', 'pct_total', 'playerid', 'season']
df_3p.columns = ['nr_player_shots', 'pct_player',
                 'x_agg', 'y_agg', 'pct_total', 'playerid', 'season']
...
df_2p['max_player_shots_any_bin'] = df_2p.groupby(['playerid', 'season'])[
    'nr_player_shots'].transform('max')
df_2p['class_nr_shots'] = 1
df_2p.loc[df_2p['nr_player_shots'] /
          df_2p['max_player_shots_any_bin'] >= 1/3, 'class_nr_shots'] = 2
df_2p.loc[df_2p['nr_player_shots'] /
          df_2p['max_player_shots_any_bin'] >= 2/3, 'class_nr_shots'] = 3
df_2p.drop('max_player_shots_any_bin', inplace=True, axis=1)

df_3p['max_player_shots_any_bin'] = df_3p.groupby(['playerid', 'season'])[
    'nr_player_shots'].transform('max')
df_3p['class_nr_shots'] = 1
df_3p.loc[df_3p['nr_player_shots'] /
          df_3p['max_player_shots_any_bin'] >= 1/3, 'class_nr_shots'] = 2
df_3p.loc[df_3p['nr_player_shots'] /
          df_3p['max_player_shots_any_bin'] >= 2/3, 'class_nr_shots'] = 3
...
final = pd.merge(final, players_df[['team', 'playerid', 'name', 'season']], how='left', left_on=[
                 'playerid', 'season'], right_on=['playerid', 'season'])
...
con = Postgres(database=DATABASE, user=USER,
